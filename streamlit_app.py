...
final_model = pickle.load(open('poly_model.pkl', 'rb'))


def predict_calories_consumed(data,user,pw,db):
    ...
    prediction = pd.DataFrame(final_model.predict(clean2), columns = ['Pred_calories_consumed'])
    
    final = pd.concat([prediction, data], axis = 1)
    final.to_sql('calconsume_predictons', con = engine, if_exists = 'replace', chunksize = 1000, index= False)
...
def main():
    st.title("Calories_consumed prediction")
    st.sidebar.title("Fuel Efficiency prediction")
    
    html_temp = """
    <div style="background-color:tomato;padding:10px">
    <h2 style="color:white;text-align:center;">Calories_consumed prediction ML App </h2>
    </div>
    """
    ...
    if st.button("Predict"):
        result = predict_calories_consumed(data, user, pw, db)
